...
export class PassPricingService {
  computePassPrice(input: PassPriceInput): PassPriceResult {
    ...
    const base =
      (config as typeof BIJOU_PRICING).ADMISSION_CZT_TOKENS_BASE ??
      (SHOWZONE_PRICING as typeof SHOWZONE_PRICING).PASS_BASE_CZT_TOKENS;
    ...
    const creator_tier_multiplier =
      input.venue === 'SHOWZONE'
        ? SHOWZONE_PRICING.CREATOR_TIER_MULTIPLIERS[input.creator_tier]
        : 1.0; // Bijou has no creator-tier multiplier
    const advance_multiplier =
      input.venue === 'SHOWZONE' ? this.resolveAdvanceMultiplier(input.days_until_show) : 1.0; // Bijou advance window not currently multiplied
    const geo_multiplier = this.resolveGeoMultiplier(input.vip_geo_tier);

    const composite =
      day_multiplier * time_multiplier * creator_tier_multiplier * advance_multiplier;
    ...
    const czt_price_usd =
      (config as typeof BIJOU_PRICING).CZT_PRICE_USD ?? SHOWZONE_PRICING.CZT_PRICE_USD;
  ...
  private resolveTimeMultiplier(hour: number, venue: VenueType): number {
    if (venue === 'BIJOU') return 1.0; // Bijou does not apply time-of-day multiplier
