 ...
 * PAYLOAD 5 — CreatorControlService, Flicker n'Flame Scoring (FFS) engine, timing + monitoring
...
describe('FfsEngine — tier resolution', () => {
  it('computes COLD when no tippers and no velocity', () => {
    const { stub } = natsStub();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const engine = new FfsEngine(stub as any);
